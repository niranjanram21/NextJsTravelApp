...
export default function Packages() {
    ...
    const handleBookNow = (id) => {
        router.push(`/packageDetail/${id}`); // Correct navigation with ID
    };
            ...
            <div className="product-container">
                <Row xs={1} md={2} className="g-4">
                    {productsToDisplay.map((product) => (
                        <Col key={product.id}>
                            <Card className="product-card position-relative z-1" style={{ height: "22rem" }}>
                                <div className="d-flex" style={{ height: "100%" }}>
                                    ...
                                    <Card.Body className="d-flex flex-column" style={{ padding: "1rem 2rem", color: "white", flex: "1" }}>
                                        ...
                                        <div className="d-flex gap-4 mt-auto">
                                            ...
                                            <button key={product.id} onClick={() => handleBookNow(product.id)}>
                                                Book Now
                                            </button>
                                        </div>
                                    </Card.Body>
